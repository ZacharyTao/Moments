...
public struct Photo: Identifiable, Equatable, Hashable {
...
//                }
                ...
                let photoCaptureProcessor = PhotoCaptureProcessor(with: photoSettings, willCapturePhotoAnimation: { [weak self] in
                    ...
                    if let data = photoCaptureProcessor.photoData {
                        var photoTemp = Photo(originalData: data)
                        let finalImage = UIImage(data: data)
                        if self?.videoDeviceInput.device.position == .front, let cgImage = finalImage?.cgImage {
                            var finalImage = UIImage(data: data)
                            finalImage = UIImage(cgImage: cgImage, scale: finalImage!.scale, orientation: .leftMirrored)
                            let finalData = finalImage?.jpegData(compressionQuality: 1.0)
                            photoTemp =  Photo(originalData: finalData ?? data)
                        }
                        self?.photo = photoTemp
                        self?.stop()
                        print("passing photo")
                    } else {
                        print("No photo data")
